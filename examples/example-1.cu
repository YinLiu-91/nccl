...
int main(int argc, char *argv[]) {
    ...
    // initializing NCCL
    NCCLCHECK(ncclCommInitAll(comms, nDev, &devs[0]));
    ...
    for (int i = 0; i < nDev; ++i) {
        NCCLCHECK(ncclAllReduce((const void *) sendbuff[i],
                                ...
                                comms[i], s[i]));
        NCCLCHECK(ncclBroadcast(sendbuff[i], recvbuff[i], size, ncclFloat, 0, comms[i], s[i]));
    }
    // CUDACHECK(ncclSend(sendbuff[0],size,ncclFloat,1,comms[0],s[0]));
